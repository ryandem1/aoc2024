...
func Day3Part2(puzzleInputName string) (string, error) {
	...
	total := 0
	disabled := false
	disableCommand := "don't()"
	enableCommand := "do()"

	for i := range puzzleInput {
		withinBound := i + len(disableCommand) <= len(puzzleInput)
		if withinBound && puzzleInput[i:i + len(disableCommand)] == disableCommand {
			disabled = true
		}

		withinBound = i + len(enableCommand) <= len(puzzleInput)
		if disabled && withinBound && puzzleInput[i:i + len(enableCommand)] == enableCommand {
			disabled = false
		}
